...
- 涉及业务规则的判断节点，直接在下方「计算/判断逻辑」章节描述，同一文档内联合维护
...
> 业务规则、计算公式、判断分支统一在本节内联维护，与所属流程同文档共存（不另设独立目录）。一个流程可包含多条规则，逐条列出。
...
# 关键参数：rate_a=0.05 / rate_b=0.03 / deduction=2
```

**边界场景**：
| 场景 | 输入 | 期望输出 |
|------|------|---------|
| 边界 1 | `amount=0` | `result=0` |
| 边界 2 | `amount` 超大 | `result=upper_limit` |
